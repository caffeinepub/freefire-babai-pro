...
function createGain(ctx, value, at = 0) {
...
function playTileFlip() {
  ...
  try {
    ...
    osc.type = "sine";
    osc.frequency.setValueAtTime(520, now2);
    osc.frequency.exponentialRampToValueAtTime(900, now2 + 0.12);
    ...
    reverb.buffer = buf;
    const gain = createGain(ctx, 0.22, now2);
    gain.gain.exponentialRampToValueAtTime(1e-3, now2 + 0.38);
    osc.connect(gain);
    gain.connect(reverb);
...
function playDragonHit() {
  ...
  try {
    ...
    rumble.frequency.setValueAtTime(80, now2);
    rumble.frequency.exponentialRampToValueAtTime(30, now2 + 0.6);
    const rumbleGain = createGain(ctx, 0.4, now2);
    rumbleGain.gain.exponentialRampToValueAtTime(1e-3, now2 + 0.7);
    const screech = ctx.createOscillator();
    screech.type = "sawtooth";
    screech.frequency.setValueAtTime(600, now2 + 0.05);
    screech.frequency.exponentialRampToValueAtTime(120, now2 + 0.5);
    const screechGain = createGain(ctx, 0.28, now2 + 0.05);
    screechGain.gain.exponentialRampToValueAtTime(1e-3, now2 + 0.6);
    ...
    rumble.start(now2);
    rumble.stop(now2 + 0.8);
    screech.start(now2 + 0.05);
    screech.stop(now2 + 0.65);
  } catch (_) {
  }
}
function playCashout() {
  ...
  try {
    ...
    freqs.forEach((freq, i) => {
      const osc = ctx.createOscillator();
      osc.type = "sine";
      osc.frequency.setValueAtTime(freq, now2 + i * 0.07);
      const g = createGain(ctx, 0, now2 + i * 0.07);
      g.gain.linearRampToValueAtTime(0.25, now2 + i * 0.07 + 0.02);
      g.gain.exponentialRampToValueAtTime(1e-3, now2 + i * 0.07 + 0.5);
      osc.connect(g);
      g.connect(ctx.destination);
      osc.start(now2 + i * 0.07);
      osc.stop(now2 + i * 0.07 + 0.55);
    });
    for (let c = 0; c < 6; c++) {
      ...
      const g = createGain(ctx, 0.1, now2 + c * 0.055);
      g.gain.exponentialRampToValueAtTime(1e-3, now2 + c * 0.055 + 0.1);
      src.connect(filter);
      filter.connect(g);
      g.connect(ctx.destination);
      src.start(now2 + c * 0.055);
    }
  } catch (_) {
  }
}
function playGameStart() {
  ...
  try {
    ...
    osc.frequency.exponentialRampToValueAtTime(440, now2 + 0.08);
    osc.frequency.exponentialRampToValueAtTime(330, now2 + 0.16);
    const g = createGain(ctx, 0.18, now2);
    g.gain.exponentialRampToValueAtTime(1e-3, now2 + 0.25);
    osc.connect(g);
    g.connect(ctx.destination);
    osc.start(now2);
    osc.stop(now2 + 0.28);
...
function playHoverTile() {
  ...
  try {
    ...
    const g = createGain(ctx, 0.04, now2);
    g.gain.exponentialRampToValueAtTime(1e-3, now2 + 0.06);
    osc.connect(g);
    g.connect(ctx.destination);
    osc.start(now2);
    osc.stop(now2 + 0.07);
...
function generateDragonPositions(count) {
...
function tileLabel(index2) {
  const row = String.fromCharCode(65 + Math.floor(index2 / 5));
  const col = index2 % 5 + 1;
  return `${row}${col}`;
}
function calcMultiplier(prev, totalTiles, safeFound, dragons) {
...
function LiveBetRow({ entry, isNew }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `dm-live-row${isNew ? " dm-live-row--new" : ""}`, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-live-name", children: entry.name }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-live-bet", children: formatCurrency(entry.bet) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "dm-live-dragons", children: [
      entry.dragons,
      "🐉"
    ] }),
...
function GameTile({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "button",
    {
      ...
      onMouseEnter: clickable ? onHover : void 0,
      disabled: !clickable,
      "aria-label": `Tile ${tileLabel(tile.index)}`,
      "data-ocid": `tile.item.${tile.index + 1}`,
      children: tile.revealed ? tile.isDragon ? /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-tile-icon dm-tile-dragon-icon", children: tile.isHit ? "💥" : "🐉" }) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-tile-icon dm-tile-gem-icon", children: "💎" }) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-tile-label", children: tileLabel(tile.index) })
    }
  );
}
function App() {
  ...
  const [betAmount, setBetAmount] = reactExports.useState(50);
  const [betInput, setBetInput] = reactExports.useState("50");
  const [dragonCount, setDragonCount] = reactExports.useState(5);
  ...
  const [gemsFound, setGemsFound] = reactExports.useState(0);
  const [multiplier, setMultiplier] = reactExports.useState(1);
  const [currentWin, setCurrentWin] = reactExports.useState(0);
  const [resultMsg, setResultMsg] = reactExports.useState("");
  const [betError, setBetError] = reactExports.useState("");
  const [mute, setMuteState] = reactExports.useState(false);
  const [liveBets, setLiveBets] = reactExports.useState(() => {
    const initial = [];
    for (let i = 0; i < 6; i++) {
  ...
  const [newestBetId, setNewestBetId] = reactExports.useState(-1);
  const liveBetIdRef = reactExports.useRef(100);
  ...
  const [withdrawMsg, setWithdrawMsg] = reactExports.useState("");
  reactExports.useEffect(() => {
  ...
  function generateFakeBet(id) {
    const won = Math.random() > 0.45;
    const dc = DRAGON_OPTIONS[Math.floor(Math.random() * DRAGON_OPTIONS.length)];
  ...
  function handleTileClick(tileIndex) {
    ...
    if (tile.isDragon) {
      ...
      setTiles(
        (prev) => prev.map((t) => t.index === tileIndex ? { ...t, revealed: true } : t)
      );
      ...
      if (newGemsFound >= safeTiles) {
        ...
        setTiles(
          (prev) => prev.map((t) => t.isDragon ? { ...t, revealed: true } : t)
        );
        setGameState("won");
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-root", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dm-bg-glow", "aria-hidden": "true" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dm-bg-smoke", "aria-hidden": "true" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "dm-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-brand", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-brand-dragon", children: "🐉" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-brand-text", children: "DRAGON MINE" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-header-right", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "button",
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-wallet-icon", children: "💰" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-wallet-balance", children: formatCurrency(balance) })
            ]
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "dm-main", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("aside", { className: "dm-panel dm-controls-panel", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-panel-inner", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-balance-display", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-balance-label", children: "YOUR BALANCE" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-balance-value", children: formatCurrency(balance) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              type: "button",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-section", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-section-label", id: "bet-label", children: "BET AMOUNT" }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-bet-row", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                type: "button",
                className: "dm-stepper-btn",
                onClick: () => handleBetInput(Math.max(MIN_BET, betAmount - 10).toString()),
                disabled: isPlaying,
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                type: "button",
                className: "dm-stepper-btn",
                onClick: () => handleBetInput(Math.min(balance, betAmount + 10).toString()),
                disabled: isPlaying,
          ...
          betError && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "dm-error-msg", "data-ocid": "bet.error_state", children: betError }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dm-quick-bets", children: QUICK_BETS.map((qb, qi) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              type: "button",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-section", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-section-label", children: "DRAGONS IN MINE" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dm-dragon-selector", children: DRAGON_OPTIONS.map((dc, di) => /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              type: "button",
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "dm-dragon-hint", children: [
            dragonCount,
            " dragons hidden in ",
            TOTAL_TILES,
            " tiles"
        ...
        isPlaying && /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "button",
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-cashout-label", children: "💎 CASH OUT" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-cashout-amount", children: formatCurrency(currentWin) })
            ]
          }
        ),
        (gameState === "won" || gameState === "lost") && /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            type: "button",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "dm-panel dm-board-panel", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-stats", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-stat", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-stat-label", children: "GEMS FOUND" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-stat-value dm-stat-gems", children: gemsFound })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-stat", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-stat-label", children: "MULTIPLIER" }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "span",
              {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-stat", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-stat-label", children: "CURRENT WIN" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-stat-value dm-stat-win", children: formatCurrency(currentWin) })
          ] })
        ] }),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dm-grid", "data-ocid": "game.canvas_target", children: tiles.map((tile) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          GameTile,
          {
            tile,
            gameState,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "dm-live-section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-section-header", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-live-dot" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "dm-section-title", children: "LIVE BETS" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-live-table", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-live-header-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "PLAYER" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "BET" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "DRAGONS" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "RESULT" })
        ] }),
        liveBets.slice(0, 7).map((entry) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          LiveBetRow,
          {
            entry,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "dm-how-section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "dm-section-title", children: "HOW TO PLAY" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-how-cards", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-how-card", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-how-num", children: "1" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-how-icon", children: "🎯" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "dm-how-title", children: "Set Bet & Dragons" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "dm-how-desc", children: "Choose your bet amount (min ₹30) and how many dragons lurk in the mine." })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-how-card", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-how-num", children: "2" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-how-icon", children: "💎" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "dm-how-title", children: "Click Tiles for Gems" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "dm-how-desc", children: "Each safe tile reveals a gem and raises your multiplier. More dragons = faster rise." })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-how-card", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-how-num", children: "3" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "dm-how-icon", children: "💰" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "dm-how-title", children: "Cash Out to Win" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "dm-how-desc", children: "Cash out anytime after your first gem. Hit a dragon and lose it all!" })
        ] })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("footer", { className: "dm-footer", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "dm-footer-demo", children: [
        "Demo mode — Min bet ₹",
        MIN_BET,
        " · Min withdrawal ₹",
        MIN_WITHDRAW
    ...
    showWithdraw && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dm-modal-overlay", "data-ocid": "withdraw.modal", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("dialog", { className: "dm-modal", open: true, "aria-label": "Withdraw funds", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "dm-modal-title", children: "💸 WITHDRAW" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "dm-modal-balance", children: [
        "Balance: ",
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: formatCurrency(balance) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "dm-modal-note", children: [
        "Minimum withdrawal: ",
      ...
      withdrawMsg && /* @__PURE__ */ jsxRuntimeExports.jsx(
        "p",
        {
          className: `dm-modal-msg${withdrawMsg.includes("requested") ? " dm-modal-msg--ok" : " dm-modal-msg--err"}`,
          "data-ocid": "withdraw.success_state",
          children: withdrawMsg
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dm-modal-actions", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            type: "button",
...
const WalletTransactionStatus = Variant({
  "pending": Null,
  "approved": Null,
  "rejected": Null
});
const Time = Int;
const WalletTransaction = Record({
  "id": Nat,
...
Service({
  ...
  "getAllUsers": Func([], [Vec(UserProfile)], ["query"]),
  "getAllUsersWithWalletOver": Func(
    [Nat],
    [Vec(UserProfile)],
    ["query"]
  ),
  "getAllWalletTransactions": Func(
    [],
    [Vec(WalletTransaction)],
    ["query"]
  ),
  "getCallerUserProfile": Func([], [UserProfile], ["query"]),
  "getCallerUserRole": Func([], [UserRole], ["query"]),
  "getMatches": Func([], [Vec(Match)], ["query"]),
  "getPendingPayments": Func([], [Vec(WalletTransaction)], ["query"]),
  "getUserProfile": Func([Principal2], [UserProfile], ["query"]),
  "getUserProfileByUsername": Func([Text], [UserProfile], ["query"]),
  "getUsersSortedByWallet": Func([], [Vec(UserProfile)], ["query"]),
  "getWalletBalance": Func([], [Nat], []),
  "getWithdrawals": Func([], [Vec(WalletTransaction)], ["query"]),
  "hasUserTransaction": Func(
    [
      Principal2,
      Variant({ "deposit": Null, "withdrawal": Null })
    ],
    [Bool],
    ["query"]
  ),
  "isCallerAdmin": Func([], [Bool], ["query"]),
  "isRegistered": Func([Principal2], [Bool], ["query"]),
  "joinMatch": Func([], [], []),
  "login": Func([Text], [Bool], []),
  "register": Func([Text, Text], [], []),
  "requestWithdraw": Func([Nat], [], []),
  "saveCallerUserProfile": Func([UserProfile], [], []),
  "submitPayment": Func([Nat], [], []),
  "updateWalletTransactionStatus": Func([Nat], [], [])
});
